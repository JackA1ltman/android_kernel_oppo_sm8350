 ...
 * probe_kernel_read() has already happened and our safe buffer is
 * populated.
 */
				  ...
				  const struct btf_type *member_type)
{
	...
	/* Different architecture may have different alignment.
	 * Use member_type size as alignment, capped at 8 bytes.
	 */
	align_bytes = min_t(u64, member_type->size, 8);
				    ...
				    u32 meta_left)
{
	...
	tag = (const struct btf_decl_tag *)(t + 1);
	/* component_idx == -1 means tagging the type itself,
	 * otherwise it tags a member/param at that index.
	 */
...
static int btf_decl_tag_resolve(struct btf_verifier_env *env,
				const struct resolve_vertex *v)
{
	/* Simply mark resolved — full validation would check that
	 * the referenced type is a struct/union/func/var/typedef.
	 */
	env_stack_pop_resolved(env, 0, 0);
	return 0;
}

static void btf_decl_tag_log(struct btf_verifier_env *env,
			      const struct btf_type *t)
{
	const struct btf_decl_tag *tag = (const void *)(t + 1);

	btf_verifier_log(env, "type=%u component_idx=%d",
			 t->type, tag->component_idx);
}
...
/* BTF_KIND_TYPE_TAG — a modifier like const/volatile, but for type annotations */
static const struct btf_kind_operations type_tag_ops = {
	.check_meta = btf_ref_type_check_meta,
	.resolve = btf_modifier_resolve,
	.check_member = btf_modifier_check_member,
	.check_kflag_member = btf_modifier_check_kflag_member,
...
static s32 btf_enum64_check_meta(struct btf_verifier_env *env,
				  ...
				  u32 meta_left)
{
	const struct btf_enum64 *member;
	u32 meta_needed;
	u16 vlen;

	vlen = BTF_INFO_VLEN(t->info);
	meta_needed = vlen * sizeof(*member);
...
static void btf_enum64_log(struct btf_verifier_env *env,
			    const struct btf_type *t)
{
	btf_verifier_log(env, "size=%u vlen=%u",
			 t->size, BTF_INFO_VLEN(t->info));
}

static int btf_enum64_check_member(struct btf_verifier_env *env,
				    const struct btf_type *struct_type,
				    const struct btf_member *member,
				    const struct btf_type *member_type)
{
...
static const struct btf_kind_operations enum64_ops = {
	...
	.resolve = btf_df_resolve,
	.check_member = btf_enum64_check_member,
	.check_kflag_member = btf_df_check_kflag_member,
	.log_details = btf_enum64_log,
	.show = btf_df_show,
};
...
static const struct btf_kind_operations * const kind_ops[NR_BTF_KINDS] = {
	...
	[BTF_KIND_TYPE_TAG] = &type_tag_ops,
